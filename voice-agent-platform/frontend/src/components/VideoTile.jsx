  ...
  return (
    <div
      className={`relative rounded-lg bg-white border border-gray-200 overflow-hidden shadow-sm ${
        fill ? 'w-full h-full min-h-0' : 'aspect-video'
      }`}
    >
      {hasVideo ? (
        <video
        ...
        <div className="w-full h-full flex items-center justify-center bg-gray-100 min-h-[120px]">
          <span className="text-gray-500 text-sm">{displayName || 'Off'}</span>
        </div>
      )}
      <span className="absolute bottom-1 left-1 px-2 py-0.5 rounded text-xs bg-black/60 text-white truncate max-w-full">
